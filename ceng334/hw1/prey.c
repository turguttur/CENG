...
int calculateManhattanDistance(int x_hunter, int y_hunter, int x_prey, int y_prey) {
	int manhattanDistance = abs(x_hunter - x_prey) + abs(y_hunter - y_prey);
...
int main(int argc, char const *argv[])
{
	...
	while(1) {
		...
		if (read(0, &message, sizeof(server_message))) {
			...
			int currentDistance = calculateManhattanDistance(message.adv_pos.x, message.adv_pos.y, currentPosx, currentPosy);
			...
			if (currentPosx + 1 < mapHeight && req_positionIsEmpty(currentPosx + 1, currentPosy, objectCount, message.object_pos)
				&& calculateManhattanDistance(message.adv_pos.x, message.adv_pos.y, currentPosx + 1, currentPosy) > currentDistance) {
				...
				&& calculateManhattanDistance(message.adv_pos.x, message.adv_pos.y, currentPosx, currentPosy + 1) > currentDistance) {
			...
			fprintf(stderr, "%s\n", "ERROR: Reading server message from server is failed! (Prey)");
		}
